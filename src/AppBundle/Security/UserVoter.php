...
class UserVoter extends Voter
{
    ...
    protected function voteOnAttribute($attribute, $subject, TokenInterface $token)
    {
        ...
        // un usuario siempre puede administrarse a sí mismo
        if ($user === $subject) {
            return true;
        }

        // obtener pertenencia del usuario actual a la organización del perfil
        ...
        // obtener pertenencia del usuario solcitado a la organización del perfil
        $membershipSubject = $this->em->getRepository('AticaCoreBundle:Membership')->findOneBy(
            [
                'user' => $subject,
                'organization' => $this->session->get('organization_id')
            ]
        ...
        if (null === $membership || null === $membershipSubject) {
            return false;
        }
